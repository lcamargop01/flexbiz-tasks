...
function renderHeader() {
  ...
  return '<header class="bg-white border-b shadow-sm sticky top-0 z-20"><div class="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">' +
    ...
    '<div class="flex items-center gap-4"><div class="hidden md:block">'+companyLabel+'</div>' +
...
function renderClientTasks() {
  return '<div class="flex flex-wrap items-center gap-3 mb-4">' +
    '<select onchange="CS.filters.status=this.value;loadTasks().then(render)" class="text-sm border rounded-lg px-3 py-2"><option value="">All Statuses</option><option value="todo"'+(CS.filters.status==='todo'?' selected':'')+'>To Do</option><option value="in_progress"'+(CS.filters.status==='in_progress'?' selected':'')+'>In Progress</option><option value="review"'+(CS.filters.status==='review'?' selected':'')+'>Review</option><option value="done"'+(CS.filters.status==='done'?' selected':'')+'>Done</option></select>' +
    '<select onchange="CS.filters.project_id=this.value;loadTasks().then(render)" class="text-sm border rounded-lg px-3 py-2"><option value="">All Projects</option>'+CS.projects.map(p=>'<option value="'+p.id+'"'+(CS.filters.project_id==p.id?' selected':'')+'>'+esc(p.name)+'</option>').join('')+'</select>' +
    renderCompanyFilter() +
    ...
    CS.tasks.map(t => '<div class="task-card bg-white rounded-xl border p-4 cursor-pointer" onclick="loadTaskDetail('+t.id+')">' +
      '<div class="flex items-center gap-3 mb-2"><div class="flex-1"><h3 class="font-semibold text-sm">'+esc(t.title)+'</h3><div class="flex items-center gap-2 mt-1">'+(t.project_name?'<span class="text-xs text-gray-500"><i class="fas fa-folder mr-1"></i>'+esc(t.project_name)+'</span>':'')+(CS.hasMultiCompany && t.client_name ? companyChip(t.client_name) : '')+'</div></div>'+priorityIcon(t.priority)+'<span class="status-badge '+statusColor(t.status)+'">'+t.status.replace('_',' ')+'</span></div>' +
